...
import { Car, Filter, Search, SlidersHorizontal, Sparkles } from "lucide-react";
import { BRANDS, CITIES, PRICE_RANGES } from "@/lib/data";
import { useState, useEffect } from "react";
import { motion } from "framer-motion";
import { useSearchParams } from "next/navigation";

function VoituresNeuvesContent() {
    ...
    // Récupérer les voitures depuis l'API (isNew=true)
    useEffect(() => {
        const fetchCars = async () => {
            try {
                setLoading(true);
                const res = await fetch(`${API_URL}/api/cars?limit=1000&isNew=true`);
    ...
    return (
        <div className="min-h-screen bg-[#F8F9FA]">
            ...
            <PageHeader
                ...
                title="Voitures"
                highlight="Neuves"
                description="Découvrez les derniers modèles disponibles chez les concessionnaires officiels et vendeurs particuliers."
            />
                    ...
                    <div className="lg:col-span-3">
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-4 mb-6 bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
                            <p className="font-medium text-gray-600">
                                <span className="text-[#1A1A2E] font-bold">{sortedCars.length}</span> véhicule{sortedCars.length > 1 ? 's' : ''} neuf{sortedCars.length > 1 ? 's' : ''}
                            </p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {loading ? (
                                ...
                                <div className="col-span-full text-center py-12">
                                    <p className="text-gray-500 text-lg">Aucun véhicule neuf trouvé.</p>
                                    <button
                                        onClick={() => {
                                            setSearchTerm("");
                                            setSelectedBrand("");
